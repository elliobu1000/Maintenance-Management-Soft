...
class Ui(QtWidgets.QMainWindow):
    ...
    def popup(self, title='Title', text='', info='', flag=QMessageBox.Information):
    ...
    def alpha(self):
        ...
        list_of_line_edits = [
            L, str(self.lineEdit_2.text()), self.comboBox.currentText(), str(self.lineEdit_3.text()),
            str(self.lineEdit_6.text()),
            str(self.lineEdit_5.text()), str(self.timeEdit_5.text()), str(self.timeEdit_2.text()),
            str(self.timeEdit.text()),
        ...
        if list_of_line_edits[0] in list([ws['B' + str(i + 1)].value for i in range(1, len(ws['B']))]):
            self.popup(text='Attention!', info=f'Le N°OT : {list_of_line_edits[0]} existe déjà',
        ...
        ws['A' + str(self.i + 1)] = dt.date.today().strftime("%m/%d/%Y")
        for i, n in enumerate(list_of_line_edits):
            ws[chr(65 + i + 1) + str(self.i + 1)] = n

        heure_fin_dintervention = dt.datetime(1990, 1, 1, int(str(self.timeEdit.text()).split(':')[0]),
                                              int(str(self.timeEdit.text()).split(':')[1]), 0)
        heure_arret_machine = dt.timedelta(hours=int(str(self.timeEdit_5.text()).split(':')[0]),
                                           minutes=int(str(self.timeEdit_5.text()).split(':')[1]))
        ...
        line_edits = [str(self.lineEdit.text()), str(self.lineEdit_2.text()), str(self.lineEdit_3.text()),
                      str(self.lineEdit_6.text()), str(self.lineEdit_5.text())]
        labels = [self.label.text(), self.label_3.text(), self.label_2.text(), self.label_4.text(),
                  self.label_10.text()]
    ...
    def calendar(self):
        ...
        mois = ['Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Aôut', 'Septembre', 'Octobre',
                'Novembre', 'Décembre']
        dates = self.to_datetime(
            [ws['A' + str(i + 2)].value for i in range(len(ws['A'])) if ws['A' + str(i + 2)].value != None])
        ...
        arret_machine = self.to_time(
            [ws['M' + str(i + 2)].value for i in range(len(ws['M'])) if ws['M' + str(i + 2)].value != None])
        equipements = list(
            set([ws['E' + str(i + 1)].value for i in range(1, len(ws['E'])) if ws['E' + str(i + 2)].value != None]))
        ...
        dates_de_pannes = [n for i, n in enumerate(dates) if
                           mois_souhaite == dates[i].month and annee_souhaite == dates[i].year]
        ...
        for d in dates:
            if mois_souhaite == d.month and annee_souhaite == d.year:
                stat.append(True)
            else:
                stat.append(False)
        # print(stat)

        heure_arret_machine = [round(arret_machine[i].hour + arret_machine[i].minute / 60, 3) for i, n in
                               enumerate(equip) if n in machines_en_panne and stat[i]]
        somme_heure_arret_machine = [sum([heure_arret_machine[i] for i, m in enumerate(machines_en_panne) if e == m])
                                     for e in equipements]

        # somme_heure_arret_machine = [sum([heure_arret_machine[i] for i, n in enumerate(machines_en_panne)]) for e in equipements]
        # somme_heure_arret_machine = [sum([heure_arret_machine[i]]) for i, m in enumerate(machines_en_panne) if m in equip]
        ...
        if MTBF == [nombre_de_jours_dans_mois for _ in range(len(n_machines_en_panne))]:
            self.popup(title='Manque de Données', text='données de ce mois ne sont pas dans la base de données')
        else:
